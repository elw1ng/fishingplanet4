...
class ClassName(BaseScript):  # Название класса (должен отличаться от других названий скриптов)
    ...
    def getNextFrame(self):
        ...
        self.fullimg = img
        self.img = img[192:(192+224),256:(256+224)]
        ...
        while True:
            ...
            while not losted:
                ...
                while not losted:
                    ...
                    if Prediction[0].probs.top1 >= 2 and ((Prediction[0].probs.top1conf> 0.65 and Prediction[0].probs.top1 ==2 ) or (Prediction[0].probs.top1conf + Prediction[0].probs.top5conf[1]> 0.75 and Prediction[0].probs.top5[1]>=2)):
                        print("PULL")
                        self.lkmpress()
                        break
                if self.checklost():
